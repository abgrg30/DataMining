...
def safe_parse(raw_json):
    try:
        objson = ujson.loads(raw_json)
    except ValueError, e:
        return 0
    
    if ('limit' not in objson) and ('user' in objson) and ('text' in objson) and ('id_str' in objson['user']):
        return objson
    else:
        return 0
    
idtext = rdd.map(lambda x:safe_parse(x)).filter(lambda x:x!=0).map(lambda x:(x['user']['id_str'], x['text'].encode('utf-8')))
...
# your code here
print_users_count(idtext.map(lambda x:(x[0],1)).reduceByKey(lambda x,y:x+y).count())
...
# your code here
import cPickle
f = open('../Data/users-partition.pickle', 'rb')
dic = cPickle.load(f)
f.close()
...
# your code here
idtext.cache()
sgc = idtext.map(lambda x: (7 if dic.has_key(x[0])==False else dic[x[0]], 1)).reduceByKey(lambda x,y:x+y)
...
# your code here
#print_post_count(sgc.collect())
#print_post_count(sgc.collectAsMap().items())
print_post_count(sgc.takeOrdered(8))
...
# your code here
idtokens = idtext.flatMapValues(lambda x : tok.tokenize(x))
unidtokens = idtokens.map(lambda x: (x,1)).reduceByKey(lambda x,y:x+y).map(lambda x:x[0])
unidtokens.cache()
uniquetokens = unidtokens.map(lambda x : (x[1],1)).reduceByKey(lambda x,y:x+y)
print_count(uniquetokens)
...
# your code here
import heapq
freqtokens = uniquetokens.filter(lambda x : x[1] >= 100)
ct = freqtokens.collectAsMap()
print_count(freqtokens)
print_tokens(heapq.nsmallest(20, ct.items(), key=lambda x:(-x[1],x[0])))
...
# your code here
freqgrouptokens = unidtokens.filter(lambda x:ct.has_key(x[1])).map(lambda x:(7 if dic.has_key(x[0])==False else dic[x[0]],x[1]))

ck = freqgrouptokens.map(lambda x : (x,1)).reduceByKey(lambda x,y:x+y)
grouptokenprob = ck.map(lambda x: (x[0][0], [(x[0][1], get_rel_popularity(x[1], ct[x[0][1]]))])).reduceByKey(lambda x,y:x+y)

result = grouptokenprob.mapValues(lambda x: heapq.nsmallest(10, x, key= lambda y:(-y[1],y[0]))).collectAsMap()

for i in xrange(8):
    print_tokens(result[i], i)